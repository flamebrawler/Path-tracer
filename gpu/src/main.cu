...
static void draw() {
    for (int j = 0; j < globalHeight; j++) {
        for (int i = 0; i < globalWidth; i++) {
            //FrameBuffer.pixel(i, j) = aliases::float3(PCG32::rand()); // noise
            //FrameBuffer.pixel(i, j) = aliases::float3(0.5f * (cos((i + globalFrameCount) * 0.1f) + 1.0f)); // moving cosine
        }
    }
}
...
static void setupLightSource() {
    //light.position = aliases::float3(0.5f, 4.0f, 1.0f); // use this for sponza.obj
    light.position = aliases::float3(3.0f, 3.0f, 3.0f);
    light.wattage = aliases::float3(1000.0f, 1000.0f, 1000.0f);
...
static void setupScene(int argc, const char* argv[]) {
    if (argc>1) {
		...
		do{
			...
			objLoadSucceed = mesh.load("E:\\Programming\\CC++\\CUDA playground\\media\\testObj.obj");
		}while(mesh.materials.size()<=4);// in case it doesn't load properly for some reason
